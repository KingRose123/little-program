...
// 用户在开发者工具里最常踩的两个坑，把原因直接写进错误提示
function netError(e) {
  const msg = String((e && e.errMsg) || '')
  if (msg.indexOf('not in domain list') > -1) {
    return '域名未加入白名单：请在微信公众平台配置 request 合法域名，或勾选「不校验合法域名」'
  }
  if (msg.indexOf('timeout') > -1) return '行情接口超时，请稍后重试'
  return msg || '网络请求失败'
}
