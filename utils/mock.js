...
// 默认资料。真实昵称与头像需要在登录流程里取（utils/api.js 的 login 目前仍是桩实现，
// 尚未接微信手机号授权），这里只给中性默认值，不放任何示例身份信息。
const user = {
  nickName: '微信用户',
...
const membershipFeatures = [
  ...
  { name: 'A股、ETF、基金持仓', free: '≤ 6 只', lite: '无限', pro: '无限' },
...
const profileMenus = {
  group1: [
    ...
    // value 由 api.getProfile() 按真实绑定情况回填（未绑定显示「未绑定」）
    { key: 'phone', icon: '📱', name: '手机号', value: '' },
    // 把当前微信账号与 App 账号并成一个（配对码，不用短信）
    { key: 'link', icon: '🔗', name: '关联 App 账号', value: '' },
...
const legalDocs = {
  ...
  agreement: {
    ...
    paras: [
      ...
      '二、账号与数据。你用微信登录后，持仓、分红记录、支出偏好等数据会同步到服务器，以便换设备或重装后恢复。数据按微信身份隔离，其他用户无法看到。你可以在「我的 → 注销账号」中随时删除全部数据。',
  ...
  privacy: {
    ...
    paras: [
      ...
      '一、不采集的部分。本应用不读取通讯录、短信、相册、地理位置，也不做通讯记录分析。',
      '二、账号标识。你使用微信登录后，我们会获得微信提供的用户标识（openid），仅用于区分账号与保存你的数据。',
      '三、手机号。经你授权，我们会获得微信绑定的手机号，用于手机号快捷登录、账号绑定与账号找回；在界面上以「138****0000」这样的脱敏形式展示，仅在本应用内使用，不用于营销，也不会提供给第三方。',
